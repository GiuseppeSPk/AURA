...
**Audit Status**: ✅ **34/34 checks PASSED** (run `python scripts/run_audit.py` to verify)

---

## Executive Summary

Partner B has completed **all data preparation, preprocessing, and evaluation infrastructure**. All 4 datasets are downloaded, processed, and verified. This document contains everything you need to start building the models.

**Quick Stats:**
- 📊 **Datasets**: 4 (GoEmotions, OLID, Jigsaw, ToxiGen) = ~87,000 samples
- 🎯 **Emotion Classes**: 7 (including neutral)
- ⚖️ **Class Weights**: Computed and ready
- 📝 **Report**: Dataset section drafted
...
10. [Complete Data File Inventory](#complete-data-file-inventory)
...
│   ├── processed/                 # ALL PROCESSED DATA (10 files, ~36MB)
...
│   ├── run_audit.py               # Comprehensive project audit
│   ├── download_olid.py           # OLID dataset pipeline
│   ├── download_jigsaw.py         # Jigsaw dataset pipeline
│   └── download_toxigen.py        # ToxiGen dataset pipeline
│
├── src/
│   ├── data/
...
| `class_weights.json` | 7 weights | For weighted loss |

**Labels**: 7 Ekman emotions (anger, disgust, fear, joy, sadness, surprise, **neutral**)
...
| File | Samples | Purpose |
|------|---------|---------|
| `toxigen_test.json` | 8,960 | Implicit hate evaluation |

**Labels**: Binary (42% toxic, 58% non-toxic)  
**Source**: GPT-3 generated (machine-written implicit hate)

**Target Groups in Dataset:**
- women, lgbtq, mental_dis, black, chinese, asian, native_american, middle_east, muslim, physical_dis...
...
### 3.1 Ekman Emotion Distribution (7 CLASSES - IMPROVED!)

**UPDATE (Jan 7, 2025)**: We added **neutral** as a 7th class instead of mapping it to joy. This significantly improved the class balance:

| Emotion | Train % | Class Weight | Notes |
|---------|---------|--------------|-------|
| **Joy** | 36.4% | 0.39 | ✅ Reduced from 66% (was inflated by neutral) |
| **Neutral** | 29.5% | 0.48 | NEW: Now separate class |
| Anger | 12.9% | 1.11 | OK |
| Surprise | 11.4% | 1.26 | OK |
| Sadness | 6.8% | 2.12 | Low |
| **Fear** | 1.6% | **8.97** | Still underrepresented |
| **Disgust** | 1.5% | **9.72** | Still underrepresented but improved |

**Improvement Summary**:
- Joy reduced from **66% → 36.4%** (neutral no longer inflates joy)
- Disgust weight reduced from **11.34 → 9.72** (relative improvement)
- More honest representation of emotion distribution
- **You MUST still use weighted loss** (weights in `class_weights.json`)
- **Focal Loss (γ=2)** still recommended for hard examples
...
class AURAModel(nn.Module):
    ...
    Architecture:
        Shared BERT Encoder
            ...
            └── Emotion Head: Linear(768 → 7)  # CHANGED: 7 classes (includes neutral)
    
    Expected Output:
        tuple: (toxicity_logits, emotion_logits)
            - toxicity_logits: (batch, 2)
            - emotion_logits: (batch, 7)  # CHANGED: 7 classes
    """
...
# emo_logits.shape = (batch, 7)  # CHANGED: 7 classes now
```
...
#     'text': 'cleaned text',
#     'ekman_label': 'anger',
#     'ekman_id': 0  # anger=0, disgust=1, fear=2, joy=3, sadness=4, surprise=5, neutral=6
# }
```
...
```python
# For emotion head (7 classes now)
with open(DATA_DIR / "class_weights.json", encoding='utf-8') as f:
    weights = json.load(f)
    
# Order: anger, disgust, fear, joy, sadness, surprise, neutral
emotion_weights = torch.tensor(weights['weights'])  # [1.11, 9.72, 8.97, 0.39, 2.12, 1.26, 0.48]

# For toxicity head (2 classes)
with open(DATA_DIR / "olid_class_weights.json", encoding='utf-8') as f:
    weights = json.load(f)
...
## 10. Complete Data File Inventory

All files are in `data/processed/` (10 files, ~36 MB total):

| File | Size | Samples | Schema |
|------|------|---------|--------|
| `goemotions_ekman_train.json` | 12.5 MB | 43,410 | `{text, ekman_label, ekman_id}` |
| `goemotions_ekman_val.json` | 1.6 MB | 5,426 | `{text, ekman_label, ekman_id}` |
| `goemotions_ekman_test.json` | 1.6 MB | 5,427 | `{text, ekman_label, ekman_id}` |
| `class_weights.json` | 0.5 KB | 7 weights | `{weights, ekman_emotions, ekman_to_id}` |
| `olid_train.json` | 4.9 MB | 11,280 | `{text, label, label_name}` |
| `olid_val.json` | 0.6 MB | 1,410 | `{text, label, label_name}` |
| `olid_test.json` | 0.6 MB | 1,410 | `{text, label, label_name}` |
| `olid_class_weights.json` | 0.1 KB | 2 weights | `{weights, labels}` |
| `jigsaw_test.json` | 10.1 MB | 10,000 | `{text, label, label_name, toxic_types}` |
| `toxigen_test.json` | 3.1 MB | 8,960 | `{text, label, label_name, target_group}` |
...
# Toxicity (label)
TOXICITY_TO_IDX = {'non-toxic': 0, 'toxic': 1}
```
