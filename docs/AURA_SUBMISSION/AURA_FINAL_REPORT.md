...
**Tecnologie per la Rappresentazione dei Dati Multimodali e Archivi (2025/2026)**
...
Questo documento racconta la storia completa del nostro lavoro: i tentativi falliti, le notti passate a debuggare, e le scelte architetturali che ci hanno portato al modello finale. 
...
Sembrava perfetto! Lavoro finito! Ma, spinti da un sano scetticismo da studenti, abbiamo eseguito dei test manuali (stress test qualitativi con frasi volgari ma non offensive) e qualcosa non ci convinceva affatto. Avevamo ragione a dubitare.
...
- **Focal Loss** (Lin et al., 2017): Cercando una soluzione valida tra i materiali del corso, e in particolare nelle lezioni dedicate ai dataset sbilanciati del **Modulo 3 (Prof. Bioglio)**, abbiamo individuato questa funzione di perdita come la chiave potenziale. Invece di usare i class weights statici, che pesano l'intera classe a priori, abbiamo implementato la Focal Loss per pesare dinamicamente i singoli esempi. Abbiamo impostato il **focusing parameter $\gamma=2.0$**, ovvero il valore "gold standard" per abbattere drasticamente la perdita sugli esempi "facili" (quelli che il modello classifica con alta confidenza) e costringere l'encoder a concentrarsi solo sui **Hard Negatives**. Questo approccio è molto più raffinato del semplice weighting, perché permette al modello di non sprecare gradienti su pattern già appresi (come la tossicità esplicita) e focalizzarsi sulle ambiguità.
...
- **Kendall Uncertainty Loss** (Kendall et al., 2018): Questo paper sarebbe stato la nostra "salvezza" metodologica per gestire il bilanciamento dinamico dei task e, soprattutto, per **gestire il rumore intrinseco dei dati**. Invece di cercare pesi fissi, abbiamo trattato il Multi-Task Learning come un problema di **inferenza bayesiana**, implementando l'**incertezza omoscedastica** ($\sigma$) come parametro apprendibile per ogni testa.
...
Abbiamo deciso di implementare questa soluzione applicando il principio della **Multi-Head Attention** studiato nel **Modulo 2 (Prof. Basile)**, in particolare il concetto di **"Redundancy"**: invece di avere un singolo meccanismo di attenzione condiviso, abbiamo creato **4 blocchi di Multi-Head Attention paralleli e specializzati** (uno per ogni task). Inizialmente eravamo stati parsimoniosi (pensando che RoBERTa avesse già abbastanza "ridondanza" interna con le sue 144 teste), ma i test qualitativi ci hanno dimostrato che forzare tutto nel vettore [CLS] creava troppa confusione. Con 4 blocchi separati, abbiamo dato ad ogni task i propri "occhi" specializzati.
...
Abbiamo risolto la guerra tra i gradienti dei vari task con una strategia a due livelli, applicando i concetti di **Advanced Learning** del **Modulo 1**:
...
*Report compilato il 28 Gennaio 2026, ore 23:59.*
